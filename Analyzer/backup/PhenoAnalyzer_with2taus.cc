...
//  TClonesArray *branchGenParticle = treeReader->UseBranch("Particle"); 
  ...
  TH1 *Numbertaus		= new TH1F("Numbertaus",	"Numbertaus", 6, 0, 6); 		 
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
    {
      ...
      pass_cuts[0]=1;
 	int countertau=0;
    	int countermother1=0;
              int countermother2=0;
	int countermother3=0;
   /*  for(int l = 0; l < branchGenParticle->GetEntriesFast(); l++)
                {
                  GenParticle *tau = (GenParticle*) branchGenParticle->At(l);
                  GenParticle *mother = (GenParticle*) branchGenParticle->At(l);

                  if ((tau->PID == abs(15) ))                              
                    {
                      countertau++;

                      countermother1=mother->M1;

			if((countermother1==17)&&tau->PID == abs(17))

			{
			countermother3=mother->D1;
			}
                      countermother2=mother->M2;

      //                cout <<"n particles per events  "<< countertau <<" M1 " << countermother1 << "   M2 " <<countermother2<<endl;

                    }
                }

              Numbertaus->Fill(countertau);
              Mothers->Fill(countermother1);
*/
      ...
      if(branchJet->GetEntries() > 0)
	{
	  ...
	  if (branchJet->GetEntriesFast() == 2) 
		...
		for (int i = 0; i < tau_index.size(); i++)
	      ...
	      if ((pass_cuts[1] == 1) && (Tau1_vec.Pt() > tau1_pt_min) && (tau_index.size()==1)){
		pass_cuts[2] = 1; 
	      }
  ...
  for (int i = 0; i < directories; i++)
    {
      ...
      _hmap_n_jets[i]        = new TH1F("N_jets",         "N(jet)", 3, 0.5, 3.5);
