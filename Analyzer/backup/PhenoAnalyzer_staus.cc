...
PhenoAnalysis::PhenoAnalysis(TChain& chain, TFile* theFile, TDirectory *cdDir[], int nDir)
{
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
	...
	Muon *muon = (Muon*) branchMuon->At(muo);
        ...
        HT_lept += muon->PT;
	if ((muon->PT > 10.) && (abs(muon->Eta) < 2.5)){nmuon_counter++;}            
      }
      /////////////////////////////////////////////////////////
      for(int ele=0; ele < branchElectron->GetEntriesFast(); ele++){
      Electron *electron = (Electron*) branchElectron->At(ele);
      ...
      for (int j = 0; j < branchJet->GetEntriesFast(); j++)
	{
	  ...
	  for (int el = 0; el < branchElectron->GetEntriesFast(); el++){
	    ...
	    elec_i.SetPtEtaPhiE(elec->PT, elec->Eta, elec->Phi, elec_energy);
            if((elec_i.Pt() > 10.) && (elec_i.Eta() < 2.5)){nelec_counter++;} //number of electrons
  ...
  for (int i = 0; i < directories; i++)
    {
      ...
      _hmap_Transverse_Mass[i] = new TH1F("Transverse_Mass",   "Transverse Mass", 60, 0, 600);
      _hmap_Transverse_Mass_1[i] = new TH1F("Transverse_Mass",   "Transverse Mass_1", 60, 0, 600);
      _hmap_met[i]           = new TH1F("met",            "met", 120, 0., 1200.); 
      _hmap_MET[i]           = new TH1F("MET",            "MET", 120, 0., 1200.);
      _hmap_MET_1[i]           = new TH1F("MET_1",            "MET_1", 120, 0., 1200.);
      _hmap_met_HT[i]        = new TH1F("met/HT",   "met/HT", 100, 0, 10.);
      _hmap_MET_HT[i]        = new TH1F("MET/HT",   "MET/HT", 100, 0, 10.);
      _hmap_MET_1_HT[i]        = new TH1F("MET_1/HT",   "MET_1/HT", 100, 0, 10.);
      _hmap_met_sqrtHT[i]        = new TH1F("met/sqrtHT",   "met/sqrtHT", 100, 0, 10.);
      _hmap_MET_sqrtHT[i]        = new TH1F("MET/sqrtHT",   "MET/sqrtHT", 100, 0, 10.);
      _hmap_MET_1_sqrtHT[i]        = new TH1F("MET_1/sqrtHT",   "MET_1/sqrtHT", 100, 0, 10.);
      _hmap_MT_on_pT_lead[i] = new TH1F("MT_on_pT_lead",  "MT_on_pT_lead", 32, 0., 4.);
