...
//		double M_jjb= TMath::Sqrt(TMath::Power((sum_energyjjb),2)-p123);
        ...
        double M_jjb = (vec_jet[p] + vec_jet[q]+bjet_i).M();
        mass_t = M_jjb;
        pt_top = pt_W+bjet_i.Pt();
		hbratio = bjet_energy/(vec_jet[p] + vec_jet[q]+bjet_i).E();
		energy_t = sum_energyjjb;
		if (mass_t <= 200. ){passed_mass_t = true;}
	      }
